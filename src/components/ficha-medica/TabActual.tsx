...
export default function TabActual({
  ...
  const formatDate = (value: unknown) => {
    if (!value) return '—';
    try {
      return formatFrontendDate(String(value), locale === 'en' ? 'en-US' : 'es-AR');
    } catch {
      return String(value);
    }
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    doc.save(`${tx('ficha-medica', 'medical-record')}-${socioLabel ?? authUser?.nombre ?? tx('socio', 'member')}.pdf`.replace(/\s+/g, '-').toLowerCase());
  };
