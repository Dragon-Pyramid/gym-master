...
> = {
  idle: {
    container:
      "border-cyan-300 bg-cyan-50 text-slate-950 dark:border-cyan-700 dark:bg-slate-950/90 dark:text-white",
    badge:
      "border-cyan-300 bg-cyan-100 text-cyan-900 dark:border-cyan-600 dark:bg-cyan-950 dark:text-cyan-100",
  ...
  success: {
    container:
      "border-emerald-300 bg-emerald-50 text-emerald-950 dark:border-emerald-700 dark:bg-emerald-950/90 dark:text-emerald-50",
    badge:
  ...
  debt: {
    container:
      "border-red-300 bg-red-50 text-red-950 dark:border-red-700 dark:bg-red-950/90 dark:text-red-50",
    badge:
  ...
  inactive: {
    container:
      "border-amber-300 bg-amber-50 text-amber-950 dark:border-amber-700 dark:bg-amber-950/90 dark:text-amber-50",
    badge:
      "border-amber-300 bg-amber-100 text-amber-900 dark:border-amber-600 dark:bg-amber-900 dark:text-amber-100",
  ...
  error: {
    container:
      "border-slate-300 bg-slate-50 text-slate-950 dark:border-slate-700 dark:bg-slate-950/90 dark:text-white",
    badge:
...
function getTerminalNeonClass(color?: string | null) {
  return (
    terminalNeonClasses[color || "verde_fluo"] || terminalNeonClasses.verde_fluo
  );
}
...
function getVariantFromAccess(
  alertType?: string,
  accessStatus?: string,
): TerminalVariant {
  if (alertType === "inactive" || accessStatus === "desactivado")
    return "inactive";
  if (alertType === "debt" || accessStatus === "deuda") return "debt";
  if (
    alertType === "error" ||
    accessStatus === "qr_expirado" ||
    accessStatus === "sin_socio"
  )
    return "error";
  if (
    alertType === "success" ||
...
function buildTerminalEventFromAsistencia(
  row: AsistenciaReciente,
): TerminalEvent {
...
function buildTerminalEventFromBroadcast(
  payload: AccessBroadcastPayload,
): TerminalEvent {
  const variant = getVariantFromAccess(
    payload.alert_type,
        ...
        : variant === "debt"
          ...
          : variant === "inactive"
            ...
            : variant === "error"
              ...
              : "Asistencia registrada correctamente. Bienvenido al gimnasio."),
    tipoMovimiento: payload.tipo_movimiento,
    timestamp: new Date(),
  };
}
...
function TerminalIcon({ variant }: { variant: TerminalVariant }) {
  ...
  if (variant === "debt")
    return <AlertTriangle className="h-20 w-20 md:h-28 md:w-28" />;
  if (variant === "inactive")
    return <ShieldAlert className="h-20 w-20 md:h-28 md:w-28" />;
...
export default function AsistenciaTerminalDisplay() {
  ...
  const sessionStateRef = useRef<TerminalSessionState>(sessionState);
  ...
  const ensureTerminalSession = useCallback(
    async (force = false) => {
      ...
      if (
        !force &&
        currentExpiresAt &&
        !isTokenCloseToExpiring(currentExpiresAt)
      ) {
        setSessionState({
          status: "ok",
  ...
  useEffect(() => {
    ...
    const loadTerminalAds = async () => {
      if (sessionStateRef.current.status === "expired") return;
  ...
  useEffect(() => {
    ...
    const frequencyMs =
      Math.max(5, Number(currentAd.terminal_frecuencia_segundos || 15)) * 1000;
    const durationMs =
      Math.max(3, Number(currentAd.terminal_duracion_segundos || 8)) * 1000;
    ...
    adShowTimeoutRef.current = window.setTimeout(() => {
      ...
      adHideTimeoutRef.current = window.setTimeout(() => {
        setShowTerminalAd(false);
        setTerminalAdIndex(
          (prev) => (prev + 1) % Math.max(terminalAds.length, 1),
        );
      }, durationMs);
    }, frequencyMs);
  ...
  const activeTerminalAd =
    terminalAds[terminalAdIndex % Math.max(terminalAds.length, 1)] ?? null;
  const shouldShowTerminalAd =
    isIdle && showTerminalAd && Boolean(activeTerminalAd);
      ...
      <section className="relative flex h-screen max-h-screen min-h-0 flex-col overflow-hidden p-3 md:p-4 2xl:p-5">
        ...
        <header className="relative z-10 flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
          <div className="flex items-center gap-3 2xl:gap-4">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-cyan-300 md:text-sm">
                Gym Master
              </p>
          ...
          <div className="flex items-center gap-3 text-right">
            <div>
              ...
              <p className="font-mono text-2xl font-bold text-white md:text-3xl 2xl:text-4xl">
                {formatTime(now)}
              </p>
            </div>
            <Button
              ...
              {shouldShowTerminalAd && activeTerminalAd ? (
                <div className="overflow-hidden rounded-[1.75rem] border border-white/15 bg-black/35 shadow-2xl">
                  ...
                  <div className="space-y-3 p-5 pt-4 text-center 2xl:space-y-4 2xl:p-6 2xl:pt-5">
                    <h3
                      className={`text-3xl font-black leading-tight 2xl:text-4xl ${getTerminalNeonClass(activeTerminalAd.terminal_color_neon)}`}
                    >
                ...
                recentItems.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-2xl border border-white/10 bg-black/25 p-3 shadow-lg 2xl:p-4"
                  >
                    <div className="flex items-center gap-3 2xl:gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <p className="truncate text-xl font-black leading-tight 2xl:text-2xl">
                            {item.nombre}
                          </p>
                          <span
