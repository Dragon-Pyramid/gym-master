...
if(!user){
  return NextResponse.json({error:"El usuario no esta logueado"},{status:400})
}
  ...
  if (!tokenAsistencia) {
    return NextResponse.json({ error: 'Falta el tokenAsistencia' }, { status: 400 })
  }
  ...
  try {
    ...
    console.error(err)
    return NextResponse.json({ valido: false, error: err.message }, { status: 401 })
  }
}
