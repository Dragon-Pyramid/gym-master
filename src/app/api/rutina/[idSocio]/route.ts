...
export async function GET(req: Request, { params }: { params: { idSocio: string } }) {
    try {
        ...
        if (!idSocio) {
            return NextResponse.json({ error: "ID del socio es requerido" }, { status: 400 });
        }

        const rutinas = await historialRutinaSocio(user, idSocio);

        if (!rutinas) {
            return NextResponse.json({ error: "Rutinas no encontradas para el socio" }, { status: 404 });
        }
