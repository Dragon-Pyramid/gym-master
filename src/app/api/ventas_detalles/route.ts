...
export async function POST(req: Request) {
  try {
    ...
    if (!body.venta_id || !body.cantidad || (!body.producto_id && !body.servicio_id)) {
      return NextResponse.json(
        { error: 'Debe informar venta, cantidad y producto o servicio' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: 'Detalle de venta creado con éxito', data: detalle },
      { status: 201 }
    );
  } catch (error: any) {
    return NextResponse.json(
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json(
      { message: 'Detalle de venta actualizado con éxito', data: detalleModificado },
      { status: 200 }
    );
  } catch (error: any) {
    return NextResponse.json(
