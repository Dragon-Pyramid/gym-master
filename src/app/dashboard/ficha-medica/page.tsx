...
function getSocioLabel(socio?: Socio | null) {
  if (!socio) return 'Socio no seleccionado';
  return socio.nombre_completo || socio.email || `Socio ${socio.id_socio}`;
}
...
export default function FichaMedicaPage() {
  ...
  useEffect(() => {
    ...
    fetchSociosApi()
      ...
      .catch(() => {
        if (cancelled) return;
        setSocios([]);
        setSociosError('No se pudo cargar el listado de socios para revisión.');
      })
      .finally(() => {
  ...
  }, [isInitialized, isAuthenticated, canReviewSocios]);
                  ...
                  <div className='flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between'>
                    <div className='max-w-3xl'>
                      <div className='inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-emerald-100'>
                        ...
                        {canReviewSocios ? 'Revisión médica operativa' : 'Mi control médico'}
                      </div>
                      <h1 className='mt-4 text-2xl font-black leading-tight sm:text-3xl'>
                        {canReviewSocios ? 'Ficha médica del socio' : 'Mi ficha médica'}
                      </h1>
                      <p className='mt-2 text-sm leading-6 text-emerald-50/90 sm:text-base'>
                        {canReviewSocios
                          ? 'Centralizá datos clínicos básicos, apto médico, adjuntos e historial para una revisión rápida antes de entrenamientos o actividades.'
                          : 'Consultá tus datos vigentes, registrá una nueva actualización y revisá tu historial médico cargado en el gimnasio.'}
                      </p>
                    </div>
                    {canReviewSocios ? (
                      <div className='grid grid-cols-2 gap-3 sm:grid-cols-4 lg:min-w-[440px]'>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          <div className='text-xs text-emerald-100'>Modo</div>
                          <div className='mt-1 text-lg font-black'>Admin</div>
                        </div>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          <div className='text-xs text-emerald-100'>Socios</div>
                          <div className='mt-1 text-lg font-black'>{socios.length}</div>
                        </div>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          <div className='text-xs text-emerald-100'>Activos</div>
                          <div className='mt-1 text-lg font-black'>{sociosActivos}</div>
                        </div>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          <div className='text-xs text-emerald-100'>Revisión</div>
                          <div className='mt-1 text-lg font-black'>{socioId ? 'Lista' : 'Pendiente'}</div>
                        </div>
                      </div>
                    ) : (
                      <div className='grid gap-3 sm:grid-cols-3 lg:min-w-[440px]'>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          ...
                          <div className='mt-1 text-sm font-bold text-white'>Datos vigentes</div>
                        </div>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          <div className='flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-emerald-100'>
                          ...
                          <div className='mt-1 text-sm font-bold text-white'>Registrar control</div>
                        </div>
                        <div className='rounded-2xl border border-white/15 bg-white/10 p-3 backdrop-blur'>
                          <div className='flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-emerald-100'>
                          ...
                          <div className='mt-1 text-sm font-bold text-white'>Ver registros</div>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className='rounded-2xl border bg-background p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/80 sm:p-5'>
                  <div className='flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between'>
                    <div>
                      ...
                      <h2 className='mt-1 text-xl font-black'>Seleccionar socio para revisar</h2>
                      <p className='mt-1 text-sm text-muted-foreground'>
                        Buscá por nombre, DNI, email o teléfono y abrí la ficha actual, historial o nueva carga del socio seleccionado.
                      </p>
                    </div>
                        ...
                        {filteredSocios.length === 0 ? (
                          ...
                          filteredSocios.map((socio) => (
                            <option key={socio.id_socio} value={socio.id_socio}>
                              {getSocioLabel(socio)}{socio.dni ? ` · DNI ${socio.dni}` : ''}
                            </option>
                          ))
                        )}
                      </select>
                  ...
                  <div className='mt-4 grid gap-3 sm:grid-cols-3'>
                    <div className='rounded-xl border bg-muted/30 p-3 dark:border-slate-800 dark:bg-slate-900/70'>
                      <div className='text-xs text-muted-foreground'>Socio seleccionado</div>
                      <div className='mt-1 truncate text-sm font-bold'>{getSocioLabel(selectedSocio)}</div>
                    </div>
                    <div className='rounded-xl border bg-muted/30 p-3 dark:border-slate-800 dark:bg-slate-900/70'>
                    ...
                    <div className='rounded-xl border bg-muted/30 p-3 dark:border-slate-800 dark:bg-slate-900/70'>
                      ...
                      <div className='mt-1 inline-flex items-center gap-2 text-sm font-bold'>
                        ...
                        {selectedSocio?.activo === false ? 'Inactivo' : 'Activo'}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className='rounded-2xl border bg-background p-3 shadow-sm dark:border-slate-800 dark:bg-slate-950/80 sm:p-4'>
                <div className='mb-4 flex flex-col gap-2 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-amber-900 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-100 sm:flex-row sm:items-start'>
                  ...
                  <p className='text-xs leading-5 sm:text-sm'>
                    La ficha médica es una herramienta administrativa y preventiva. No reemplaza diagnóstico ni indicación profesional. Ante alertas clínicas, derivar a un profesional de salud.
                  </p>
                </div>
                <Tabs
                  socioId={socioId}
                  socioLabel={canReviewSocios ? getSocioLabel(selectedSocio) : user?.nombre ?? 'Socio'}
