                ...
                <CardContent>
                  <form onSubmit={handleCreatePack} className='space-y-4'>
                    <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                      <div className='space-y-1.5'><Label htmlFor='comercial-pack-nombre'>{c('Nombre')}</Label><Input id='comercial-pack-nombre' value={packNombre} onChange={(e) => setPackNombre(e.target.value)} required /></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-pack-precio'>{c('Precio final')}</Label><Input id='comercial-pack-precio' type='number' min='0' step='0.01' value={packPrecio} onChange={(e) => setPackPrecio(e.target.value)} required /></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-pack-canal'>{c('Canal')}</Label><select id='comercial-pack-canal' className='h-10 w-full rounded-md border px-3 text-sm' value={packCanalId} onChange={(e) => setPackCanalId(e.target.value)}>{dashboard.canales.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-pack-grupo'>{c('Grupo')}</Label><select id='comercial-pack-grupo' className='h-10 w-full rounded-md border px-3 text-sm' value={packGrupoId} onChange={(e) => setPackGrupoId(e.target.value)}>{dashboard.grupos.map((g) => <option key={g.id} value={g.id}>{g.nombre}</option>)}</select></div>
                    </div>
                    <div className='space-y-1.5'><Label htmlFor='comercial-pack-descripcion'>{c('Descripción')}</Label><Input id='comercial-pack-descripcion' value={packDescripcion} onChange={(e) => setPackDescripcion(e.target.value)} placeholder={c('Ej: pack 8 clases + evaluación inicial')} /></div>
                    <div className='space-y-3'>
                      {packItems.map((item, index) => (
                        <div key={index} className='grid grid-cols-1 gap-3 rounded-lg border bg-muted/20 p-3 md:grid-cols-[0.55fr_1.2fr_0.35fr_0.45fr_auto] md:items-end'>
                          <div className='space-y-1.5'><Label htmlFor={`comercial-pack-item-${index}-tipo`}>{c('Tipo')}</Label><select id={`comercial-pack-item-${index}-tipo`} className='h-10 w-full rounded-md border px-3 text-sm' value={c(item.item_tipo === "servicio" ? "Servicio" : "Producto")} onChange={(e) => updatePackItem(index, { item_tipo: e.target.value as 'producto' | 'servicio' })}><option value='servicio'>{c('Servicio')}</option><option value='producto'>{c('Producto')}</option></select></div>
                          <div className='space-y-1.5'><Label htmlFor={`comercial-pack-item-${index}-source`}>{c('Ítem')}</Label><select id={`comercial-pack-item-${index}-source`} className='h-10 w-full rounded-md border px-3 text-sm' value={item.source_id} onChange={(e) => updatePackItem(index, { source_id: e.target.value })} required><option value=''>{c('Seleccionar')}</option>{item.item_tipo === 'producto' ? dashboard.productos.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>) : dashboard.servicios.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}</select></div>
                          <div className='space-y-1.5'><Label htmlFor={`comercial-pack-item-${index}-cantidad`}>{c('Cant.')}</Label><Input id={`comercial-pack-item-${index}-cantidad`} type='number' min='1' value={item.cantidad} onChange={(e) => updatePackItem(index, { cantidad: e.target.value })} /></div>
                          <div className='space-y-1.5'><Label htmlFor={`comercial-pack-item-${index}-referencia`}>{c('Ref.')}</Label><Input id={`comercial-pack-item-${index}-referencia`} type='number' min='0' step='0.01' value={item.precio_referencia} onChange={(e) => updatePackItem(index, { precio_referencia: e.target.value })} /></div>
              ...
              <Card>
                ...
                <CardContent>
                  <form onSubmit={handleCreatePromo} className='space-y-4'>
                    <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                      <div className='space-y-1.5'><Label htmlFor='comercial-promo-nombre'>{c('Nombre')}</Label><Input id='comercial-promo-nombre' value={promoNombre} onChange={(e) => setPromoNombre(e.target.value)} required /></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-promo-tipo'>{c('Tipo')}</Label><select id='comercial-promo-tipo' className='h-10 w-full rounded-md border px-3 text-sm' value={promoTipo} onChange={(e) => setPromoTipo(e.target.value as any)}><option value='descuento_porcentaje'>{c('% descuento')}</option><option value='descuento_fijo'>{c('$ descuento')}</option><option value='combo'>{c('Combo')}</option><option value='beneficio'>{c('Beneficio')}</option></select></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-promo-valor'>{c('Valor')}</Label><Input id='comercial-promo-valor' type='number' min='0' step='0.01' value={promoValor} onChange={(e) => setPromoValor(e.target.value)} required /></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-promo-canal'>{c('Canal')}</Label><select id='comercial-promo-canal' className='h-10 w-full rounded-md border px-3 text-sm' value={promoCanalId} onChange={(e) => setPromoCanalId(e.target.value)}>{dashboard.canales.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-promo-grupo'>{c('Grupo')}</Label><select id='comercial-promo-grupo' className='h-10 w-full rounded-md border px-3 text-sm' value={promoGrupoId} onChange={(e) => setPromoGrupoId(e.target.value)}>{dashboard.grupos.map((g) => <option key={g.id} value={g.id}>{g.nombre}</option>)}</select></div>
                      <div className='grid grid-cols-2 gap-2'><div className='space-y-1.5'><Label htmlFor='comercial-promo-inicio'>{c('Inicio')}</Label><Input id='comercial-promo-inicio' type='date' value={promoInicio} onChange={(e) => setPromoInicio(e.target.value)} /></div><div className='space-y-1.5'><Label htmlFor='comercial-promo-fin'>{c('Fin')}</Label><Input id='comercial-promo-fin' type='date' value={promoFin} onChange={(e) => setPromoFin(e.target.value)} /></div></div>
                    </div>
                  ...
                  <form onSubmit={handleCreateCupon} className='mt-6 space-y-4 rounded-xl border p-4'>
                    ...
                    <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                      <div className='space-y-1.5'><Label htmlFor='comercial-cupon-promocion'>{c('Promoción')}</Label><select id='comercial-cupon-promocion' className='h-10 w-full rounded-md border px-3 text-sm' value={cuponPromocionId} onChange={(e) => setCuponPromocionId(e.target.value)}>{dashboard.promociones.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-cupon-codigo'>{c('Código')}</Label><Input id='comercial-cupon-codigo' value={cuponCodigo} onChange={(e) => setCuponCodigo(e.target.value.toUpperCase())} placeholder='VERANO10' required /></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-cupon-max-usos'>{c('Máx. usos')}</Label><Input id='comercial-cupon-max-usos' type='number' min='1' value={cuponMaxUsos} onChange={(e) => setCuponMaxUsos(e.target.value)} /></div>
                      <div className='space-y-1.5'><Label htmlFor='comercial-cupon-expira'>{c('Expira')}</Label><Input id='comercial-cupon-expira' type='date' value={cuponExpira} onChange={(e) => setCuponExpira(e.target.value)} /></div>
                    </div>
