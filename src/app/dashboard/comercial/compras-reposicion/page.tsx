...
export default function ComercialComprasReposicionPage() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      toast.error(error.message || 'No se pudo cargar compras/reposición');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSaveRelation(event: React.FormEvent) {
    ...
    try {
      ...
      toast.error(error.message || 'No se pudo asociar proveedor');
    } finally {
    ...
    try {
      const detalles: CreateOrdenCompraDetalleDTO[] = ordenItems
        ...
        .map((item, index) => {
          ...
          if (!Number.isInteger(cantidad) || cantidad <= 0) throw new Error(`Cantidad inválida en ítem ${index + 1}`);
          if (!Number.isFinite(costo) || costo < 0) throw new Error(`Costo inválido en ítem ${index + 1}`);
          return { producto_id: item.producto_id, cantidad_solicitada: cantidad, costo_unitario: costo };
        });
      if (!detalles.length) throw new Error('La orden debe tener al menos un producto');
  ...
  async function handleReceiveOrder(orden: ComercialOrdenCompra) {
    ...
    try {
      ...
      toast.error(error.message || 'No se pudo recibir la orden');
    } finally {
      setReceivingId(null);
    }
  }
  ...
  return (
    <SidebarProvider>
      <div className='flex min-h-screen w-full'>
        <AppSidebar />
        <SidebarInset>
          <AppHeader title='Compras / Reposición' />
          <main className='flex-1 space-y-6 p-6'>
            <section className='rounded-2xl border bg-white p-6 shadow-sm'>
              <div className='flex flex-col justify-between gap-4 lg:flex-row lg:items-center'>
                <div className='space-y-2'>
                  ...
                  <h1 className='text-2xl font-bold'>Compras, proveedores y reposición</h1>
                  <p className='max-w-3xl text-sm leading-relaxed text-muted-foreground'>
                    Gestioná proveedores por producto, compras sugeridas, órdenes de compra y recepción de mercadería integrada al Stock Ledger.
                  </p>
                </div>
                <div className='flex flex-wrap gap-2'>
                  ...
                  <Button asChild variant='outline'><Link href='/dashboard/comercial/stock-ledger'>Stock Ledger</Link></Button>
            ...
            <section className='grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5'>
              <MetricCard title='A reponer' value={String(dashboard.metricas.productosAReponer)} description='Productos bajo mínimo u objetivo.' icon={AlertTriangle} />
              <MetricCard title='Costo sugerido' value={formatCurrencyARS(dashboard.metricas.costoReposicionSugerida)} description='Estimación de reposición.' icon={ShoppingCart} />
              <MetricCard title='Órdenes abiertas' value={String(dashboard.metricas.ordenesAbiertas)} description='Pedidas o parciales.' icon={ClipboardList} />
              <MetricCard title='Parciales' value={String(dashboard.metricas.ordenesParciales)} description='Con recepción pendiente.' icon={PackagePlus} />
              <MetricCard title='Proveedores' value={String(dashboard.metricas.proveedoresActivos)} description='Activos para compras.' icon={Truck} />
            </section>

            <section className='grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_0.9fr]'>
              <Card>
                <CardHeader><CardTitle>Reposición sugerida</CardTitle></CardHeader>
                <CardContent className='space-y-3'>
                  {sugerenciasPendientes.length === 0 ? (
                    <div className='rounded-lg border border-dashed p-4 text-sm text-muted-foreground'>No hay productos sugeridos para reposición.</div>
                  ) : (
                    sugerenciasPendientes.slice(0, 12).map((item) => (
                      <div key={item.producto_id} className='grid grid-cols-1 gap-3 rounded-lg border p-3 lg:grid-cols-[1.4fr_0.7fr_0.7fr_auto] lg:items-center'>
                        <div>
                          ...
                          <p className='text-xs text-muted-foreground'>Stock {item.stock_total} · mínimo {item.stock_minimo} · objetivo {item.stock_objetivo}</p>
                          <p className='text-xs text-muted-foreground'>{item.proveedor_sugerido_nombre ?? 'Sin proveedor sugerido'}</p>
                        </div>
                        <div><StockStateBadge estado={item.estado_stock} /></div>
                        <div className='text-sm'>
                          <p className='font-semibold'>Comprar {item.cantidad_sugerida}</p>
              ...
              <Card>
                <CardHeader><CardTitle>Proveedor por producto</CardTitle></CardHeader>
                <CardContent>
                  <form onSubmit={handleSaveRelation} className='space-y-4'>
                    ...
                    <div className='grid grid-cols-1 gap-3 md:grid-cols-3'>
                      <div className='space-y-1.5'><Label>Costo</Label><Input type='number' min='0' step='0.01' value={relCosto} onChange={(e) => setRelCosto(e.target.value)} required /></div>
                      <div className='space-y-1.5'><Label>Compra mínima</Label><Input type='number' min='1' value={relCompraMinima} onChange={(e) => setRelCompraMinima(e.target.value)} /></div>
                      <div className='space-y-1.5'><Label>Lead time días</Label><Input type='number' min='0' value={relLeadTime} onChange={(e) => setRelLeadTime(e.target.value)} /></div>
                    </div>
              ...
              <CardContent>
                <form onSubmit={handleCreateOrder} className='space-y-5'>
                  <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
                    ...
                    <div className='space-y-1.5'><Label>Fecha orden</Label><Input type='date' value={ordenFecha} onChange={(e) => setOrdenFecha(e.target.value)} /></div>
                    <div className='space-y-1.5'><Label>Recepción estimada</Label><Input type='date' value={ordenFechaEstimada} onChange={(e) => setOrdenFechaEstimada(e.target.value)} /></div>
                  </div>
                  ...
                  <div className='space-y-3'>
                    {ordenItems.map((item, index) => (
                      <div key={index} className='grid grid-cols-1 gap-3 rounded-lg border bg-muted/20 p-3 lg:grid-cols-[1.4fr_0.45fr_0.55fr_0.55fr_auto] lg:items-end'>
                        ...
                        <div className='space-y-1.5'><Label>Cantidad</Label><Input type='number' min='1' value={item.cantidad_solicitada} onChange={(e) => updateItem(index, { cantidad_solicitada: e.target.value })} required /></div>
                        <div className='space-y-1.5'><Label>Costo</Label><Input type='number' min='0' step='0.01' value={item.costo_unitario} onChange={(e) => updateItem(index, { costo_unitario: e.target.value })} required /></div>
                        <div className='text-sm font-semibold'>{formatCurrencyARS(toNumber(item.cantidad_solicitada) * toNumber(item.costo_unitario))}</div>
                        <Button type='button' variant='ghost' onClick={() => setOrdenItems((prev) => prev.filter((_, idx) => idx !== index))} disabled={ordenItems.length === 1}>Quitar</Button>
                      </div>
                    ))}
                  </div>
                  <div className='flex flex-col justify-between gap-3 rounded-lg border bg-sky-50 p-3 md:flex-row md:items-center'>
                    ...
                    <div className='flex gap-2'>
                      <Button type='button' variant='outline' onClick={() => setOrdenItems((prev) => [...prev, createEmptyItem()])}>Agregar ítem</Button>
                  ...
                  <div className='space-y-1.5'>
                    ...
                    <Input value={ordenObservaciones} onChange={(e) => setOrdenObservaciones(e.target.value)} placeholder='Notas internas de compra o reposición' />
                  </div>
                </form>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader><CardTitle>Órdenes abiertas y recepción</CardTitle></CardHeader>
              <CardContent className='space-y-4'>
                {ordenesAbiertas.length === 0 ? (
                  <div className='rounded-lg border border-dashed p-4 text-sm text-muted-foreground'>No hay órdenes abiertas.</div>
                ) : (
                    ...
                    <div key={orden.id} className='space-y-3 rounded-xl border p-4'>
                      <div className='flex flex-col justify-between gap-3 lg:flex-row lg:items-start'>
                        <div>
                          ...
                          <p className='text-sm text-muted-foreground'>{orden.proveedor?.nombre ?? 'Proveedor'} · {estadoLabel(orden.estado)} · {formatCurrencyARS(orden.total_estimado)}</p>
                          <p className='text-xs text-muted-foreground'>Destino: {orden.ubicacion_destino?.nombre ?? 'Sin ubicación fija'}</p>
                        </div>
                          ...
                          return (
                            <div key={detalle.id} className='grid grid-cols-1 gap-3 rounded-lg bg-muted/30 p-3 md:grid-cols-[1.4fr_0.5fr_0.5fr_0.5fr] md:items-center'>
                              <div>
                                ...
                                <p className='text-xs text-muted-foreground'>Solicitado {detalle.cantidad_solicitada} · recibido {detalle.cantidad_recibida} · pendiente {pendiente}</p>
                              </div>
              ...
              <CardContent className='space-y-2'>
                {dashboard.ordenes.slice(0, 10).map((orden) => (
                  <div key={orden.id} className='flex flex-col justify-between gap-2 rounded-lg border p-3 md:flex-row md:items-center'>
                    <div>
                      <p className='font-semibold'>{orden.numero_orden}</p>
                      <p className='text-sm text-muted-foreground'>{orden.proveedor?.nombre ?? 'Proveedor'} · {estadoLabel(orden.estado)}</p>
                    </div>
