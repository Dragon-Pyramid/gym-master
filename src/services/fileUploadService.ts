...
export const uploadFile = async ( file:FileUploadDTO, folder: string) :Promise<string> => {

    if(file.size > 5 * 1024 * 1024) { // 5 MB limite
        throw new Error("File size excede los 5 MB limite.");
    }
    const url = await uploadFileCloudinary(file.buffer,file.originalName,folder);
    return url;
    }
