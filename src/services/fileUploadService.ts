...
export const uploadFile = async ( file:FileUploadDTO, dbName:string, rol:string) :Promise<string> => {

    if(file.size > 5 * 1024 * 1024) { // 5 MB limite
        throw new Error("File size excede los 5 MB limite.");
    }

    const url = await uploadFileCloudinary(file.buffer,file.originalName,dbName,rol);
    return url;
    }
