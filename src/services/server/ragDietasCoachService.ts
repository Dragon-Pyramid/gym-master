...
export async function buildDietasRagContext(
  ...
  if (!config.enabled) {
    return {
      ...
      warnings: [
        aiGeneratedContentTx(
          locale,
          'RAG desactivado. Se usa generación local segura.',
          'RAG is disabled. Safe local generation is used.',
        ),
      ],
    };
  }
  ...
  if (
    (config.provider === 'github' && !config.githubToken) ||
    (config.provider === 'openai' && !config.openaiApiKey)
  ) {
    warnings.push(
      aiGeneratedContentTx(
        locale,
        'RAG configurado parcialmente. Se usa generación local segura.',
        'RAG is partially configured. Safe local generation is used.',
      ),
    );
  }
  ...
  if (error) {
    return {
      ...
      warnings: [
        aiGeneratedContentTx(
          locale,
          'No se pudo consultar el RAG de dietas. Se usa generación local segura.',
          'The diet RAG service could not be queried. Safe local generation is used.',
        ),
        ...highRiskWarnings,
      ],
    };
